...
func SetBg(file string) error {
	regKey, err := registry.OpenKey(registry.CURRENT_USER, `Control Panel\Desktop`, registry.SET_VALUE)
	if err != nil {
		return err
	}
	defer regKey.Close()
	...
	err = regKey.SetStringValue("TileWallpaper", "0")
	if err != nil {
		return err
	}
	err = regKey.SetStringValue("WallPaper", file)
	if err != nil {
		return err
	}

	ok := SystemParametersInfo(SPI_SETDESKWALLPAPER, 0, nil, SPIF_UPDATEINIFILE|SPIF_SENDWININICHANGE)
	if !ok {
